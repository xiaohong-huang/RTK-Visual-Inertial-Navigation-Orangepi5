...
map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> FeatureTracker::trackImage(double _cur_time, const cv::Mat& _img, const cv::Mat& _img1) {
    ...
    if (prev_pts.size() > 0) {
        ...
        {
            TicToc t_o;
            cv::UMat prev_img_umat,cur_img_umat;
            prev_img.copyTo(prev_img_umat);
            cur_img.copyTo(cur_img_umat);
            
            vector<float> err;
            if (hasPrediction) {
                cur_pts = predict_pts;
                //the open_cl do not support the calcOpticalFlowPyrLK with initial estimation
                ...
                if (succ_num < 10)
                    cv::calcOpticalFlowPyrLK(prev_img_umat, cur_img_umat, prev_pts, cur_pts, status, err, cv::Size(21, 21), 3);
            } else
                cv::calcOpticalFlowPyrLK(prev_img_umat, cur_img_umat, prev_pts, cur_pts, status, err, cv::Size(21, 21), 3);
            // reverse check
            if (FLOW_BACK) {
...
#if USE_OPEN_CL
                cv::UMat cur_img_umat;
                cur_img.copyTo(cur_img_umat);
                cv::goodFeaturesToTrack(cur_img_umat, n_pts, MAX_CNT - cur_pts.size(), 0.01, MIN_DIST, mask);
#else
    ...
    if (!_img1.empty() && stereo_cam) {
        ...
        if (!cur_pts.empty()) {
            ...
            {
                cv::UMat cur_img_umat;
                cv::UMat rightImg_umat;
                cur_img.copyTo(cur_img_umat);
                rightImg.copyTo(rightImg_umat);
                
                TicToc t_check;
                vector<float> err;
                // cur left ---- cur right
                cur_right_pts = cur_pts;
                cv::calcOpticalFlowPyrLK(cur_img_umat, rightImg_umat, cur_pts, cur_right_pts, status, err, cv::Size(21, 21), 3);
                // reverse check cur right ---- cur left
                if (FLOW_BACK) {
                    cv::calcOpticalFlowPyrLK(rightImg_umat, cur_img_umat, cur_right_pts, reverseLeftPts, statusRightLeft, err, cv::Size(21, 21), 3);
                    for (size_t i = 0; i < status.size(); i++) {
